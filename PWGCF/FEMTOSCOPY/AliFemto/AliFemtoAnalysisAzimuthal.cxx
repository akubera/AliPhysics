...
AliFemtoAnalysisAzimuthal::AliFemtoAnalysisAzimuthal(unsigned int binsVertex, double minVertex, double maxVertex,
  ...
  fFemtoParticleCut(nullptr),
  fFlowParticleCut(nullptr),
  ...
  fPsi(a.fPsi)
{
  ...
  for (const auto *cf : *a.fCorrFctnCollection) {
    ...
    AliFemtoCorrFctn* fctn = cf->Clone();
...
void AliFemtoAnalysisAzimuthal::ProcessEvent(const AliFemtoEvent* hbtEvent)
{
  ...
  if (tmpPassEvent) {
    ...
    for (auto *storedEvent : *MixingBuffer()) {
  ...
  for (auto *cf : *fCorrFctnCollection) {
    cf->EventEnd(hbtEvent);
  }
}

//_______________________________________________________________________________
void AliFemtoAnalysisAzimuthal::MakePairs(const char* typeIn,
...
TVector2 AliFemtoAnalysisAzimuthal::GetQVector(AliFemtoParticleCollection* particlecollection) const
{
  ...
  for (const auto *flowparticle : *particlecollection) {
    const double
      pt = flowparticle->Track()->Pt(),
      phi = flowparticle->FourMomentum().Phi();

    mQx += pt * std::cos(2 * phi);
    mQy += pt * std::sin(2 * phi);
  }

  return TVector2(mQx, mQy);
}

//__________________________________________________
double AliFemtoAnalysisAzimuthal::GetCurrentReactionPlane() const
{
  return fPsi;
}
